...
function wordFrequency(txt) {
    ...
    var newArray = [], wordArr;
    wordArray.forEach((word) => {
        wordArr = newArray.filter((w) => {
            // return w.text == word;
            return w[0] == word;
        });
        if (wordArr[1]!==0) {
...
let handleEventStreamMessage = function (messageJson) {
    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            const wordFreq = wordFrequency(transcription).sort((a, b) => a.size < b.size );
            ...
            WordCloud(document.getElementById('word-cloud'), { list: wordFreq });
